---
title: "WDDA FS 2026: Leitfaden für Aufgabenserie 4"
date: "`r Sys.Date()`"
output:
  pdf_document:
    fig_width: 7
...
Dieser Leitfaden bietet detaillierte Erklärungen für die Übungen in WDDA FS 2026 Aufgabenserie 4. Für jede Aufgabe werden wir den Denkprozess (und die notwendigen Schritte in R) durchgehen und erklären, wie man zur richtigen Lösung gelangt.
...
Für Teil (a) sind zwei Lesarten möglich, je nachdem wie die Formulierung "Bereich plausibler Werte ... unter Verwendung der Standardfehler" interpretiert wird:

- **Variante 1 (Fehlermarge = Standardfehler):** Der angegebene Standardfehler wird direkt als Fehlermarge verwendet, analog zu Teil (b). Dies entspricht dem Ansatz in der Musterlösung des Aufgabenblatts.
- **Variante 2 (95%-Konfidenzintervall nach der "95%-Regel"):** Die Fehlermarge wird als $2 \cdot SE$ angesetzt, was näherungsweise einem 95%-Konfidenzintervall entspricht.

Beide Varianten sind verbreitet; Teil (b) gibt die Fehlermarge bereits direkt an, sodass dort keine Ambiguität besteht.
...
# Variante 1: Fehlermarge = Standardfehler (wie Musterlösung Aufgabenblatt)
lower_v1 <- x_bar - se_mean
upper_v1 <- x_bar + se_mean
cat("Variante 1 (x̄ ± SE):       ", lower_v1, "bis", upper_v1, "\n")

# Variante 2: 95%-KI mit 2 * SE
...
2. **Ergebnisse:**
   - (a) Für $\mu$:
     - Variante 1 (Musterlösung Aufgabenblatt, x̄ ± SE): [22, 28]
     - Variante 2 (95%-Konfidenzintervall, x̄ ± 2·SE): [19, 31]
...
# 95%-Konfidenzintervall berechnen
ci_95 <- quantile(bootstrap_diff$Male, c(0.025, 0.975), na.rm = TRUE)
