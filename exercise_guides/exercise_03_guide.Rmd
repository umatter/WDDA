...
1. **Überlegungen anstellen:**  
   Bei rechtsschiefen Daten – etwa wenn einzelne Personen extrem hohe Bargeldbeträge besitzen – wird der Mittelwert tendenziell durch Ausreißer nach oben verzerrt, während der Median robuster ist.
...
1. **Mittelwert berechnen:**  
   Verwenden Sie `mean()` auf der entsprechenden Variable (z. B. `gift` oder wie immer diese Variable benannt ist).

```{r}
mean_gift <- mean(data$gift, na.rm = TRUE)  # Variable ggf. anpassen
...
# Entferne NA-Werte vor dem Plotten
valid_transport <- !is.na(data$distance) & !is.na(data$transport)
if(sum(valid_transport) > 0) {
  boxplot(data$distance[valid_transport] ~ as.factor(data$transport[valid_transport]),
...
# Entferne NA-Werte vor dem Plotten
valid_eye_foot <- !is.na(data$foot) & !is.na(data$eye)
if(sum(valid_eye_foot) > 0) {
  boxplot(data$foot[valid_eye_foot] ~ as.factor(data$eye[valid_eye_foot]),
...
1. **Berechnungen:**  
   Angenommen, die Bewertungen sind in der Variable `IHG` gespeichert.

```{r}
mean_IHG <- mean(data$IHG, na.rm = TRUE)
median_IHG <- median(data$IHG, na.rm = TRUE)
quantiles <- quantile(data$IHG, probs = c(0.25, 0.75, 0.85), na.rm = TRUE)
iqr_IHG <- IQR(data$IHG, na.rm = TRUE)

cat("Mittelwert IHG:", mean_IHG, "\n")
cat("Median IHG:", median_IHG, "\n")
cat("Q1:", quantiles[1], " Q3:", quantiles[2], " IQR:", iqr_IHG, "\n")
cat("85. Perzentil:", quantiles[3], "\n")
```

2. **Interpretation:**  
   - (a) Erwartete Werte: Mittelwert ≈ 4.16, Median ≈ 4.2.  
   - (b) Da keine offensichtlichen Ausreißer vorhanden sind, ist der Mittelwert ein gutes Mass.  
...
4. **Erwartete Ergebnisse:**  
   ...
   - Sommer: Spannweite = 20, IQR = 11, Varianz ≈ 44.49, SD ≈ 6.67, VK ≈ 26.05%  
...
4. **Erwartete Ergebnisse:**  
   ...
   - Nur Frauen: ca. 0.01  
   - Nur Männer: ca. 0.12  
   Dies illustriert, wie der aggregierte Datensatz (Simpsonsches Paradoxon) andere Korrelationen ergeben kann als getrennte Gruppen.
...
22. Die Tabelle "Stock 2008" enthält prozentuale Rückgänge der Aktien zwischen Januar und Anfang Oktober.  
   ...
   (c) Enthalten die Daten Ausreißer? Konstruieren Sie ein Boxplot.  
...
```{r}
mean_stock <- mean(data$Stock2008, na.rm = TRUE)
median_stock <- median(data$Stock2008, na.rm = TRUE)
quantiles_stock <- quantile(data$Stock2008, probs = c(0.25, 0.75), na.rm = TRUE)
...
```{r}
boxplot(data$Stock2008,
        main = "Boxplot: Stock 2008",
        ylab = "Prozentuale Änderung")
```

3. **Perzentil für Belgien:**  
   Die Lösung zeigt, dass Belgien im 71. Perzentil liegt.
...
```{r}
# Angenommen, die Spalten heißen 'DAX' und 'CAC40'
