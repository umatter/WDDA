...
2. **Streuungsmasse**: Beschreiben die Variabilität oder Dispersion der Daten (z.B. Varianz, Standardabweichung)
3. **Zusammenhangsmasse**: Beschreiben die Beziehung zwischen zwei oder mehr Variablen (z.B. Kovarianz, Korrelation)

# Vorbereitung

```{r vorbereitung, eval=TRUE, message=FALSE, warning=FALSE}
# Pakete laden
...
Der arithmetische Mittelwert ist die Summe aller Datenwerte geteilt durch die Anzahl der Beobachtungen. Er ist das am häufigsten verwendete Lagemass, hat jedoch den Nachteil, dass er empfindlich gegenüber Ausreissern ist.
...
# Vergleich von Median und Mittelwert bei Ausreissern
print(paste("Median mit Ausreisser:", median(salary_with_outlier)))
print(paste("Mittelwert mit Ausreisser:", round(mean(salary_with_outlier), 2)))
```

**Statistische Erklärung**: Der Median ist ein robustes Lagemass, das die Daten in zwei gleich grosse Hälften teilt. Im Gegensatz zum Mittelwert wird er kaum von Ausreissern beeinflusst, da er nur von der Rangordnung der Daten abhängt, nicht von ihren exakten Werten. Der Median ist besonders geeignet für:
...
2. **Ungleiche Stichprobengrössen**: Wenn verschiedene Teilstichproben unterschiedlich gross sind
...
Streuungsmasse beschreiben, wie stark die Daten um das Zentrum (Lagemass) verteilt sind. Sie geben Aufschluss über die Variabilität oder Dispersion der Daten und sind wichtig, um die Homogenität oder Heterogenität eines Datensatzes zu beurteilen.
...
**Statistische Erklärung**: Die Spannweite gibt einen ersten Eindruck von der Streuung der Daten, ist aber sehr empfindlich gegenüber Ausreissern. Sie wird oft als ergänzende Information zu robusteren Streuungsmassen wie dem Interquartilsabstand angegeben. Die Spannweite ist besonders nützlich für:
- Einen schnellen Überblick über die Gesamtstreuung
- Die Identifikation möglicher Ausreisser
...
Quartile teilen die geordneten Daten in vier gleiche Teile. Der Interquartilsabstand (IQR) ist die Differenz zwischen dem dritten Quartil (Q3) und dem ersten Quartil (Q1) und umfasst die mittleren 50% der Daten. Er ist ein robustes Streuungsmass, das nicht von Ausreissern beeinflusst wird.
...
# Identifikation von Ausreissern mit der 1.5*IQR-Regel
...
- Die **1.5*IQR-Regel** wird häufig verwendet, um Ausreisser zu identifizieren:
  - Werte < Q1 - 1.5*IQR oder > Q3 + 1.5*IQR gelten als Ausreisser
...
3. Der Korrelationskoeffizient ist empfindlich gegenüber Ausreissern.
...
Boxplots (auch Box-Whisker-Plots genannt) zeigen die Verteilung der Daten über ihre Quartile. Sie sind besonders nützlich zum Erkennen von Ausreissern und zum Vergleich von Gruppen.
...
3. **Whiskers (Antennen)**:
   - In der Standardimplementierung in R: Erstrecken sich bis zum äussersten Datenpunkt innerhalb von 1.5*IQR von Q1 bzw. Q3.
